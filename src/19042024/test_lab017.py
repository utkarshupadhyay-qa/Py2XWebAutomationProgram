...
def test_vwologin_negative_tc():
    ...
    # Works only with anchor tag ( a tag )
    ...
    # App
    # ment
    # Checks for Contains - keyword
    # Find the first unique element which is best matching

    # make_appointment_btn = driver.find_element(By.PARTIAL_LINK_TEXT, "Appointment")
    # make_appointment_btn.click()

    # By TagName
    list_of_a_tags = driver.find_elements(By.TAG_NAME, "a")   # multiple elements with tags 'a' ,
                                                                    # that's why we have used .find_elements
    make_appointment_btn = list_of_a_tags[5]  # [5] - because list starts from 0 so its 6th tag
    make_appointment_btn.click()
